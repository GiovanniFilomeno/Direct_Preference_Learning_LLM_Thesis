...
# -*- coding: utf-8 -*-
"""
comparison.py — Gold Master (Multi-Seed Support)
Include esperimenti A, B, C, D, E, F, G, H, I (Drift), O (New Goal).
Supporta esecuzione su seed multipli e salvataggio gerarchico (outputs/seed=X/test_id).
"""
...
try:
    ...
    print(f"[ERROR] Impossibile importare 'maze_env'. Verifica che {SRC_DIR} contenga il codice sorgente.")
    sys.exit(1)

def set_seed(seed: int = 42):
    """Imposta il seed globale per random, numpy e torch."""
    random.seed(seed)
    np.random.seed(seed)
    try:
        import torch
...
def _generate_positions_fading(env: MazeEnv, n: int, exponential: bool = False, cutoff: float = 1.0) -> np.ndarray:
    pts = []
    pbar = tqdm(total=n, desc=f"gen fading pos (exp={exponential}, cut={cutoff})", leave=False)
    while len(pts) < n:
...
def _generate_positions_tube(env: MazeEnv, n: int) -> np.ndarray:
    ...
    pts = []
    pbar = tqdm(total=n, desc="generating tube positions", leave=False)
...
def _sample_pairs(df: pd.DataFrame,
    ...
    trials = 0
    pbar = tqdm(total=num_pairs, desc="sampling pairs", leave=False)
...
# Training DPO
# ---------------------------------------------------------------------
def _train_dpo(preferences_path: str, out_model_path: str, norm_stats_path: str, cfg: TrainConfig, seed: int, device: Optional[str] = None) -> Dict:
    import torch
    ...
    # Split deterministico basato sul seed
    train_df = df_pref.sample(frac=0.90, random_state=seed)
    temp_df  = df_pref.drop(train_df.index)
    val_df   = temp_df.sample(frac=0.50, random_state=seed)
    test_df  = temp_df.drop(val_df.index)

    # Calcolo statistiche normalizzazione LOCALI per questo seed
    # e salvataggio nel path specifico passato (dentro seed=X/test_id/)
    print(f"[DPO] Calcolo statistiche normalizzazione locali...")
    ...
    for epoch in range(cfg.epochs):
        ...
        # Log ridotto per evitare spam eccessivo in multi-seed
        if (epoch+1) % 10 == 0:
...
def run_experiment(exp: Experiment, seed: int, outputs_dir: str):
    # === SETUP DEL SEED ===
    set_seed(seed)
    
    # Costruzione path specifico: outputs_dir/seed=X/test_id
    # Nota: outputs_dir qui è la root passata dal main. Noi creiamo la sottostruttura.
    out_dir = os.path.join(outputs_dir, f"seed={seed}", exp.test_id)
    os.makedirs(out_dir, exist_ok=True)
    
    print(f"--- Running {exp.test_id} [Seed {seed}] -> {out_dir} ---")
    ...
    # File statistiche LOCALE (dentro seed=X/test_id/)
    local_norm_stats_path = os.path.join(out_dir, "norm_stats.npz")
    
    # Passiamo il seed per assicurare splitting deterministico
    hist = _train_dpo(pref_path, model_path, local_norm_stats_path, exp.train, seed=seed)

    fig = plt.figure(figsize=(7,4))
    plt.plot(hist["train_losses"], label="train")
    plt.plot(hist["val_losses"],   label="val")
    plt.xlabel("epoch"); plt.ylabel("loss"); plt.title(f"DPO Losses [{exp.test_id}] Seed={seed}")
    ...
    import torch
    # Caricamento stats locali
    if os.path.isfile(local_norm_stats_path):
        _norm = np.load(local_norm_stats_path)
        ...
        print(f"⚠️  Stats non trovate in {local_norm_stats_path}!")
    ...
    axes[0].set_title(f"Distance-to-goal (Seed {seed})", fontsize=14)
    ...
    axes[1].set_title(f"Trajectories (Seed {seed})", fontsize=14)
    ...
    report = {
        ...
        "paths": {
            ...
            "model": model_path,
            "norm_stats": local_norm_stats_path,
    ...
    print(f"[{exp.test_id}] Seed {seed} finito. Report in {os.path.join(out_dir, 'report.json')}")
...
def build_experiments() -> Dict[str, Experiment]:
    ...
    E["E3_cutoff_80"] = Experiment("E3_cutoff_80", 
                                   dataset=DatasetConfig(n_random=6000, fading_density=True, cutoff_y=0.6), 
...
def main():
    parser = argparse.ArgumentParser(description="Pipeline comparativa preferenze/DPO (Multi-Seed)")
    ...
    parser.add_argument("--seeds", type=str, default="42", help="Seed singolo (es. 42) o lista separata da virgole (es. 42,10,123)")
    ...
    try:
        ...
        print("Errore: --seeds deve contenere numeri interi separati da virgola.")
        return

    # Creazione output base
    os.makedirs(args.outputs, exist_ok=True)
    
    print(f"=== INIZIO PIPELINE ===")
    print(f"Test da eseguire: {to_run}")
    print(f"Seeds da eseguire: {seeds}")
    
    total_start = time.time()
    
    for seed in seeds:
        print(f"\n{'='*60}")
        print(f">>> ESECUZIONE SEED {seed}")
        print(f"{'='*60}")
        
        for tid in to_run:
            t0 = time.time()
            # Passiamo il seed e la cartella base. run_experiment creerà la sottocartella seed=X/test_id
            run_experiment(exps[tid], seed=seed, outputs_dir=args.outputs)
            print(f"   [{tid}] seed {seed} completato in {time.time() - t0:.1f}s")
            
    print(f"\n=== TUTTI GLI ESPERIMENTI COMPLETATI in {time.time() - total_start:.1f}s ===")
